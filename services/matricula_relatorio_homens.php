...
where matricula.academia={$id} and genero='Masculino'";
...
function getMatriculaData($conexao, $id, $dataI, $dataF) {
	...
	$sql = "select count( matricula.id) as soma
from matricula inner join aluno on (matricula.aluno = aluno.idaluno)
where matricula.academia={$id} and genero='Masculino' and data_inicio >= cast('{$dataI}'  as date) and data_inicio <= cast('{$dataF}' as date) order by date(data_inicio) desc";
