...
if (isset($_GET['treinoPadraoCategoria'])) {
	$id = $_GET['categoriaPadrao'];
	$lista = '{"result":[' . json_encode(treinoPadraoCategoria($conexao, $id)) . ']}';
	echo $lista;
}
...
function treinoPadraoCategoria($conexao, $id) {
	$treinos = array();

	$resultado = mysqli_query($conexao, "select * from treinoPadrao where categoria=$id");
